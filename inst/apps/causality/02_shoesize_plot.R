    ...
    output$distPlot <- renderPlot(
      {
        ...
        model <- summary(stats::lm(income ~ schoesize, data = df_confounder))
        modelr2 <- round(model$r.squared, 2)

        model_c <- summary(stats::lm(income ~ schoesize + sex, data = df_confounder))
        modelrc <- round(model_c$r.squared, 2)
        ...
        if (input$boolmethod == TRUE) {
          ...
          ggplot2::ggplot(
            ...
            ggplot2::geom_point() +
            ggplot2::theme_bw(base_size = 18) +
            ggplot2::labs(caption = paste("R-Squared =", modelrc))
        }
      },
      res = 96
    )
